...
log.new = function(config, standalone)
    ...
    local outfile = vim.F.if_nil(config.outfile, string.format("%s/%s.log", vim.fn.stdpath("log"), config.plugin))
    ...
    -- Add a set_level method to adjust logging level at runtime
    obj.set_level = function(new_level)
        if levels[new_level] then
            config.level = new_level
        else
            obj.warn("Invalid log level:", new_level)
        end
    end
